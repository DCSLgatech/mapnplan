...
//					findLRNode2(m_nodes[next_point_id2].first, key, key_coord);
//					std::cout<<m_nodes[next_point_id2].first<<std::endl;
//					std::cout<<m_cost[next_point_id2]/pow(m_nodes[next_point_id2].second,3)*pow(m_tree.getResolution(),3)<<std::endl;
//					std::cout<<1-m_epsilon/pow(m_nodes[next_point_id2].second,3)*pow(m_tree.getResolution(),3)<<std::endl;
//					std::cout<<key_coord<<std::endl;
//					for(int kk=0;kk<8;++kk){
//						std::cout<<m_tree.search(key)->childExists(kk)<<std::endl;
//					}
					if(!hasChildrenLRNode(m_nodes[next_point_id2].first)){
...
//						std::cout<<"breaking"<<std::endl;
			...
			if(next_point_id==m_end_index){
				...
				for(int i=mv_fwd;i<result->length();++i){
					geometry_msgs::Point p;
...
bool MSP3D::run(){
	...
	if(m_path_found){
		...
		std::cout << "nb iteration " << m_nb_step << std::endl;
		ROS_INFO("path found");
		return true;
	}else{
...
bool MSP3D::hasChildrenLRNode(octomap::point3d& pt){
	...
	double size=m_tree.getNodeSize(0);
	bool result=false;
	while(node->hasChildren() && !result){
		bool updated=false;
		if((pt-pos).norm()<m_eps){
